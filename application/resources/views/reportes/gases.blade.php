...
<div class="container-main">
    ...
    <form action="{{ route('reportes.ambiental.store', $proforma) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control @error('codigo_reporte') is-invalid @enderror"
                               name="codigo_reporte" value="{{ old('codigo_reporte', 'UIA-REP-GS-'.last(explode('-', $proforma->codigo)).'/'.now()->format('y')) }}"
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" class="form-control @error('tipo_medicion') is-invalid @enderror"
                               name="tipo_medicion" value="{{ old('tipo_medicion', $info['tipo_medicion'] ?? '') }}" placeholder="Ej: Puntual, Continuo">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('equipo_usado') is-invalid @enderror"
                               name="equipo_usado" value="{{ old('equipo_usado', $info['equipo_usado'] ?? '') }}" placeholder="Ej: Detector de gases">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered" id="tabla-puntos" style="border-color: #ffc107;">
                        ...
                        <tbody id="puntos-body">
                            ...
                            @php $r = $rg[$pi] ?? []; @endphp
                            @php $codigo = $r['codigo'] ?? $p['codigo'] ?? (count($rg) === 0 && count($puntos) === 0 ? 'GS-' . str_pad($pi + 1, 2, '0', STR_PAD_LEFT) : ''); @endphp
                            <tr class="fila-punto">
        ...
        <div class="card section-card general" style="border: 1px solid #ffc107; border-radius: 10px; margin-bottom: 1.5rem;">
            ...
            <div class="card-body">
                <textarea class="form-control @error('observaciones_gases') is-invalid @enderror"
                          name="observaciones_gases" rows="4" placeholder="Observaciones y comentarios técnicos generales...">{{ old('observaciones_gases', $reporte->observaciones_gases ?? $reporte->comentarios ?? '') }}</textarea>
                @error('observaciones_gases')<div class="text-danger small">{{ $message }}</div>@enderror
            </div>
        </div>
...
<script>
    ...
    let idx = {{ max($hasRg ? count(old('resultados_gases', $rg)) : 0, $puntosCount) ?: max($numMuestras, 1) }};
