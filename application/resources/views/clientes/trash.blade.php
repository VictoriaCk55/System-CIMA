...
<div class="container-main">
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h1>
                    <i class="fas fa-trash-alt" style="color: #2798F5;"></i>
                    Papelera de Clientes
                </h1>
                <p class="page-subtitle">
                    Clientes eliminados que pueden ser restaurados (Solo Administrador)
                </p>
            </div>
        ...
        <div class="d-flex">
            ...
            <div>
                ...
                <p style="color: #856404; margin-bottom: 0; line-height: 1.5;">
                    Al hacer clic en el botón <span style="background-color: #dc3545; color: white; padding: 0.2rem 0.5rem; border-radius: 0.25rem; font-weight: 600;">Eliminar</span> el cliente será <strong>BORRADO DEFINITIVAMENTE</strong> del sistema.<br>
        ...
        <div class="card-body">
            @if($clientes->count() > 0)
                <div class="alert alert-info mb-3">
                    ...
                    <strong>Área exclusiva para administradores.</strong> Aquí puede restaurar clientes eliminados o eliminarlos permanentemente.
                </div>
                        ...
                        <tbody>
                            @foreach($clientes as $cliente)
                                <tr>
                                    ...
                                    <td><code>{{ $cliente->nit ?? 'N/A' }}</code></td>
                                    <td>
                                        ...
                                        <div class="btn-group" role="group">
                                            ...
                                            <form action="{{ route('clientes.restore', $cliente->id) }}" method="POST" class="d-inline">
                                                ...
                                                <button type="submit" class="btn btn-sm btn-success" 
                                                        data-bs-toggle="tooltip" title="Restaurar cliente"
                                                        onclick="return confirm('¿Está seguro de restaurar este cliente?')">
                                                    <i class="fas fa-trash-restore me-1"></i> Restaurar
                                                </button>
                                            </form>
                                            @endcan
                                            @can('forzar eliminar clientes')
                                            <form action="{{ route('clientes.force-delete', $cliente->id) }}" method="POST" class="d-inline" id="delete-form-{{ $cliente->id }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" 
                                                        ...
                                                        onclick="confirmarEliminacionPermanente({{ $cliente->id }}, '{{ $cliente->razon_social }}')">
                ...
                <div class="text-center mt-3 text-muted">
                    <small>
                        ...
                        Total de clientes en papelera: {{ $clientes->total() }}
                    </small>
                </div>
            @else
                <div class="text-center py-5">
                    ...
                    <p class="text-muted mb-4">No hay clientes eliminados para mostrar.</p>
                    <a href="{{ route('clientes.index') }}" class="btn" style="background-color: #2798F5; color: white; border-radius: 30px; padding: 10px 25px;">
                        <i class="fas fa-arrow-left me-2"></i>
                        Volver a clientes activos
                    </a>
                </div>
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-center">
                    ...
                    <p class="fw-bold">Estás a punto de eliminar PERMANENTEMENTE el cliente:</p>
                    <p class="fs-5 text-danger fw-bold">"${nombre}"</p>
                    <p class="text-danger mt-2">⚠️ Esta acción NO SE PUEDE DESHACER</p>
                    <p class="small text-muted">El cliente será borrado definitivamente de la base de datos y no podrá ser recuperado.</p>
                </div>
            `,
            icon: 'warning',
